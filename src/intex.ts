...
const bookContent = document.getElementById("bookContent") as HTMLDivElement;
...
const bookList: IBook[] = JSON.parse(localStorage.getItem("bookList") || "[]");
...
interface IRender {
  renderBookList(bookList: IBook[]): string | undefined;
}
      ...
      bookElement.innerHTML = `
       <div class="max-w-sm  bg-gray-800 rounded-lg overflow-hidden transition-transform shadow-2xl border border-gray-300 hover:scale-102 hover:shadow-3xl">
        ...
        <div class="w-full overflow-hidden">
          <img src="${
            book.bookImage
          }" alt="Book Image" class="w-full h-[250px] object-cover">        
        </div>

        <div class="p-4">
          <h2 class="text-xl font-bold mb-1">${book.bookName}</h2>
          <p class="text-gray-400 mb-1">Author: <span class="text-gray-200">${
            book.bookAuthor
          }</span></p>
          <p class="text-gray-400 mb-1">Genre: <span class="text-gray-200 capitalize">${
            book.bookGenre
          }</span></p>
          <p class="text-gray-400 mb-1">Price: <span class="text-gray-200">$${book.bookPrice.toFixed(
            2
          )}</span></p>
          <p class="text-gray-400 mb-1">Stock: <span class="text-gray-200">${
            book.bookStock
          }</span></p>
          <p class="text-gray-400 mb-3">Status: <span class="text-green-400" id="bookStatus">${
            book.bookStatus
          }</span></p>
          ...
          <button ${
            book.bookStock === 0 ? "disabled" : ""
          } id="sellBtn" class="bg-blue-500 text-white py-2  w-full rounded-lg shadow hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">Sell</button>
...
const handleDeleteBook = (e: Event) => {
  ...
  switch(element.id){
    case "deleteBtn":
      ...
      const userAgreeToDelete = confirm(
...
const handleCreateBook = (e: Event) => {
  e.preventDefault();
  const inputs = <NodeList>bookForm.querySelectorAll("input , select");
  ...
  const newBook = new Book(
    ...(values as [string, string, string, string, number, number])
  );
  console.log(newBook);
